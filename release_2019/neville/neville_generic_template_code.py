...
df_nh["outcome"] = np.where(df_nh["SSCAMIGG"] >= 0.7500000, 1, 0) # Create New Column Based on Conditions with nested If then where DM = 2, PD = 1, Neither = 0
df_nh = df_nh.drop(columns = ["SSCAMIGG", "SSCAMIGA", "SSCAMIGM", "SSSALIGG", "SSSALIGG", "SSSALIGG"]) # Remove variables for Identification
df_nh = df_nh.drop(columns = ["SEQN", "SDMVPSU", "WTMEC2YR", ]) # Remove variables for Identification

df_nh = df_nh[(df_nh["LBXGH"] >= 5.7) & (df_nh["LBXGH"] <= 6.4)] # Subset data frame by USPSTF screening values
...
text_file = open("Neville/neville_generic_template_results.txt", "w") # Open text file and name with subproject, content, and result suffix
...
text_file.write("GENERIC: Template Code Script\n") # Line of text with space after
