...
df_acs = read.csv("_data/flcharts_acs_top3_stage.csv") # Import dataset from _data folder

### Step 2: Prepare Data for Classificaiton

## Resolve missing data
df_acs = df_acs %>% mutate_if(is.factor, as.numeric) # Change character to numeric values
df_acs = na.omit(df_acs) # Omit rows with NA from Data Frame

## Create new column as sum of others
df_acs$Vascular = df_acs$Heart.Diseases + df_acs$Cerebrovascular.Diseases +  df_acs$Essen.Hypertension...Hypertensive.Renal.Dis + df_acs$ Aortic.Aneurysm...Dissection + df_acs$Atherosclerosis  # Create new column based on conditions
df_acs$Metabolic = df_acs$Diabetes.Mellitus + df_acs$Nephritis..Nephrotic.Syndrome..Nephrosis # Create new column based on conditions
df_acs$Cancer = df_acs$In.Situ..Benign..Uncertain.and.Unknown.Behavior.Neoplasms + df_acs$Malignant.Neoplasm.or.Cancer
df_acs = subset(df_acs, select = -c(Heart.Diseases, Cerebrovascular.Diseases, Essen.Hypertension...Hypertensive.Renal.Dis, Aortic.Aneurysm...Dissection, Atherosclerosis, Diabetes.Mellitus, Nephritis..Nephrotic.Syndrome..Nephrosis, In.Situ..Benign..Uncertain.and.Unknown.Behavior.Neoplasms, Malignant.Neoplasm.or.Cancer))

## Change Outcome label
df_acs$outcome = df_acs$Vascular
df_acs = subset(df_acs, select = -c(Cancer, Metabolic, Vascular))

## Verify Data
dim(df_acs) # Dimensions of data frame
glimpse(df_acs)
...
df_fa = subset(df_acs, outcome > 19.430) # Subset for outcome of interest
df_fa = subset(df_fa, select = -c(Tract, State)) # Remove outcome variable
...
file = file("cedric_flcharts_fa_rf_log_auc_results.txt") # Open result file in subproject repository
