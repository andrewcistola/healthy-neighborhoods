...
df_2 = pd.read_csv("_data/acs_county_stage.csv") # Import dataset from _data folder
...
## Pull County FIPS from Census Tract
df_1["Tract"] = df_1["Tract"].astype(str)
df_1["CountyFIPS"] = df_1["Tract"].str[:5]
df_1 = df_1.apply(pd.to_numeric, errors = "coerce") # Convert all columns to numeric

## Join by CountyFIPS
df_join = pd.merge(df_1, df_2, on = "CountyFIPS", how = "left")

## Export to CSV
df_join.to_csv(r"_data/flcharts_50_county_join.csv") # Clean in excel and select variable
