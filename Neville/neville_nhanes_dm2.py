#### Healthy Neighborhoods Project: Using Data to Improve Community Health
...
## National Health and Nutrition Examination Survey 2015-2016: Base Code Script by DrewC!
...
df_nh = pd.read_csv("_bigdata/hnb_neville_nhanes_raw.csv", encoding = "ISO-8859-1", low_memory = False) # Import dataset with outcome and ecological variable for each geographical id, all datasets in _data folder in repository

## Prepare for Classification
df_nh = df_nh.apply(pd.to_numeric, errors = "coerce") # Convert all columns to numeric
df_nh = df_nh.dropna() # Drop all rows with NA values
df_out = df_nh[""] # Create dataframe of isolated outcome variable
...
features = df_nh.columns.drop(["Tract", "Diabetes"]) # Drop outcome variable and Geo to isolate all predictor variable names as features
