...
## Create dummy variables for subset of interest
df_nh["Campylobacter"] = np.where((df_nh["SSCAMIGA"] >= 0.481) | (df_nh["SSCAMIGG"] >= 0.229) | (df_nh["SSCAMIGM"] >= 0.375), 1, 0) # Create New Column Based on Conditions with nested If then
...
df_nh = df_nh.drop(columns = ["ELIGSTAT", "MORTSTAT", "UCOD_LEADING", "PERMTH_EXM", "PERMTH_INT"]) # Remove variables for Identification
...
df_nh["outcome"] = np.where((df_nh["Cancer"] == 1) & (df_nh["Campylobacter"] == 1), 1, 0) # Create New Column Based on Conditions with nested If then
df_nh = df_nh.drop(columns = ["Cancer", "Campylobacter"]) # Remove variables for Identification

## Get demographics of outcome group
df_desc = df_nh[(df_nh["outcome"] == 1)] # Subset data frame by USPSTF screening values
df_desc = df_desc.filter(["RIDAGEYR", "RIAGENDR", "RIDRETH1", "INDHHINC"]) # Keep only selected columns
df_desc.describe()
...
text_file.write("NHANES 2005-2006: Campylobacter Anitbodies and 10 Year Cancer Mortality\n") # Line of text with space after
...
df_pca = pd.DataFrame(x, columns = df_pca.columns) # convert the normalized features into a tabular format with the help of DataFrame.

## Create PCA model to determine Components
pca = PCA(n_components = Nout) # you will pass the number of variables saved above as total components to make PCA model
pca.fit(df_pca) # fit to data
df_com = pd.DataFrame(pca.explained_variance_) # Print explained variance of components
df_com = df_com[(df_com[0] > 1)]
components = len(df_com.index) # Save count of values for Variable reduction

### Step 5: Run PCA with principal component count

## PCA to reduce variables
pca = PCA(n_components = components) # you will pass the number of components to make the updated PCA model
pca.fit_transform(df_pca) # finally call fit_transform on the aggregate data to create PCA results object
df_load = pd.DataFrame(pca.components_, columns = df_pca.columns) # Export eigenvectors to data frame. These represent the coefficient attached the feature in the linear model used to develop the PC. 

## Collect list important features
df_abs = df_load.abs() # Get absolute value of eigenvectors
df_pc = pd.DataFrame(df_abs.max()) # select maximum value for each feature
df_pc = df_pc.reset_index() # Save index as first column named "index"
df_pc = df_pc.rename(columns = {"index": "Features", 0: "Loadings"}) # Rename columns
df_pc = df_pc.sort_values(by = ["Loadings"], ascending = False) # Sort Columns by Value
df_pc = df_pc[(df_pc["Loadings"] > df_pc["Loadings"].mean())] # Subset by Gini values higher than 
...
forest = rfc(n_estimators = 10000, max_depth = 10) #Use default values except for number of trees. 10000 consistently provides consistent results for 2-5k variables wihout overwhelming computaitonal time.  
...
selector = rfe(estimator = Log_RFE, step = 1, min_features_to_select = 10) # define selection parameters, in this case all features are selected. See Readme for more ifo
...
df_score["Score"] = round((df_score["Coefficient"]), 2)
...
## Save selected Features to list
features = list(df_rfe["Features"]) # Save chosen featres as list
df_log = df_nh.filter(features) # Keep only selected columns from rfe with oriignal data before NA imputation
df_log["outcome"] = df_nh["outcome"] # Add outcome variable
df_log = df_log.dropna() # Drop all observations with NA values
...
## Write Summary to Text File
text_1 = str(df_log.shape)
text_2 = str(df_score) # Save variable as string value for input below
text_3 = str(score_log) # Save variable as string value for input below
text_file = open("Neville/neville_nhanes_crc_results.txt", "a") # Open text file and name with subproject, content, and result suffix
text_file.write("\nRegression Model\n") # Line of text with space after
text_file.write(text_1) # write string version of variable above
text_file.write("\n\nCoefficient Scores\n") # Line of text with space after
