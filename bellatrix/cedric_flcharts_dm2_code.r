...
## Group by County for internal Gini
tib_gini = df_fl %>% group_by(County) %>% summarise(ineq(Vascular, type = "Gini")) # Group By Columns and Average
df_gini = as.data.frame(tib_gini) # Convert tiblle to Data Frame
colnames(df_gini) <- c("County", "Gini") # Change Column Names
df_gini$County = toupper(df_gini$County)

## Export County Gini Data
write.csv(df_gini, "_data/cedric_vasc_gini.csv") #clean in excel and select variable
...
## Group by County for internal Gini
tib_gini = df_fl %>% group_by(County) %>% summarise(ineq(Cancer, type = "Gini")) # Group By Columns and Average
df_gini = as.data.frame(tib_gini) # Convert tiblle to Data Frame
colnames(df_gini) <- c("County", "Gini") # Change Column Names
df_gini$County = toupper(df_gini$County)

## Export County Gini Data
write.csv(df_gini, "_data/cedric_can_gini.csv") #clean in excel and select variable