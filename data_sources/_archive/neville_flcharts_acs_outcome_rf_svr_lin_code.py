...
## Census Tract FL Data: Base Code Script by DrewC!
...
df_eco = pd.read.csv("acs_fl_eco_stage.csv", encoding = "ISO-8859-1") # Selected Economic characertistics of the US Populaiton, American Communtiy Survey
df_hou = pd.read.csv("acs_fl_house_stage.csv", encoding = "ISO-8859-1") # Selected Housing characertistics of the US Populaiton, American Communtiy Surve
df_soc = pd.read.csv("acs_fl_social_stage.csv", encoding = "ISO-8859-1") # Selected Social characertistics of the US Populaiton, American Communtiy Surve
...
df_m1 = pd.merge(df_eco, df_hou, on = "CensusTract", how = "inner") # Merge 1st and 2nd dataset
df_fl = pd.merge(df_m2, df_soc, on = "CensusTract", how = "inner") # Merge 3rd with 1-2

## Join Outcome Datasets
df_m1 = pd.merge(df_out1, df_out2, on = "CensusTract", how = "inner") # Merge 1st and 2nd dataset
df_out = pd.merge(df_m1, df_out3, on = "CensusTract", how = "inner") # Merge 3rd with 1-2
